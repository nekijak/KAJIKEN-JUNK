...
function jstWrap(data, template) {
  return jstmpl.jstProcess(new jstmpl.JsEvalContext(data), template);
}
...
function testJseval() {
  ...
  jstmpl.jstProcess(ctx, document.getElementById('testJseval1'));
  assertEquals("testJseval1", 1, counter);

  jstmpl.jstProcess(ctx, document.getElementById('testJseval2'));
...
function testJstLoadTemplate() {
  ...
  jstmpl.jstLoadTemplate_(document, '<div id="' + id + '">content</div>', wrapperId);
  ...
  jstmpl.jstLoadTemplate_(document, '<div id="' + id2 + '">content</div>', wrapperId);
...
function testJstGetTemplateFromDom() {
  ...
  // Success
  element = jstmpl.jstGetTemplate('t1');
  assertTrue("Asserted jstGetTemplate('t1') to return a dom element",
             !!element);
  // Failure
  element = jstmpl.jstGetTemplate('asdf');
...
function testJstGetTemplateFromFunction() {
  ...
  // Success
  element = jstmpl.jstGetTemplate('template',
...
function testPrepareNode() {
  ...
  var t = document.getElementById('t1');
  var caches = [];
  caches.push(jstmpl.JstProcessor.prepareNode_(t));
  caches.push(jstmpl.JstProcessor.prepareNode_(t));
  ...
  jstmpl.jstLoadTemplate_(document, '<div id="' + id + '" jsskip="1"></div>');
  node = document.getElementById(id);
  var cache = jstmpl.JstProcessor.prepareNode_(node);
  try {
...
function testPrepareNodeWithNoAttributes() {
  ...
  jstmpl.jstLoadTemplate_(document, '<div id="' + id + '"></div>');
  var node = document.getElementById(id);
  assertEquals('prepareNode with no jst attributes should return default',
               jstmpl.JstProcessor.jstcache_[0], jstmpl.JstProcessor.prepareNode_(node));
}
...
function testJsVars() {
  ...
  jstmpl.jstProcess(context, template);
...
function testCacheReuse() {
  ...
  assertEquals(template.firstChild.getAttribute(jstmpl.ATT_jstcache),
               template.lastChild.getAttribute(jstmpl.ATT_jstcache));
}
