...
function jstest(reprocess) {
  ...
  var input = jstmpl.jsEval(jstext);
  var t;
  if (reprocess) {
    t = elem('out');
  } else {
    elem('tc').innerHTML = elem('template').value;
    t = jstmpl.jstGetTemplate('t');
    elem('out').innerHTML = '';
    elem('out').appendChild(t);
  }
  if (profiling) Profiler.reset();
  jstmpl.jstProcess(new jstmpl.JsEvalContext(input), t);
...
function perf1(data) {
  elem('out').innerHTML = '';
  var t = jstmpl.jstGetTemplate('t1');
  elem('out').appendChild(t);
  if (profiling) Profiler.reset();
  jstmpl.jstProcess(new jstmpl.JsEvalContext(data), t);
  if (profiling) Profiler.dump();
}
...
function perf1a(data) {
  if (profiling) Profiler.reset();
  jstmpl.jstProcess(new jstmpl.JsEvalContext(data), elemOrDie('out'));
  if (profiling) Profiler.dump();
}
...
function jsperf(count) {
  ...
  if (profiling && !jstmpl.JstProcessor.profiling_) {
    jstmpl.JstProcessor.profiling_ = true;
    Profiler.monitorAll(proto(jstmpl.JstProcessor), false);
  }
